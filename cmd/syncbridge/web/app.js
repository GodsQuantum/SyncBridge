    ...
    return `<div class="job ${dis?'disabled':''}">
      <div class="job-row">
        ...
        <div class="job-act">
          ...
          <button class="btn sm ghost" onclick="toggleHist(${j.id})" title="Historique" aria-label="Historique">🕒</button>
          <button class="btn sm ghost" onclick="edit(${j.id})" title="Éditer" aria-label="Éditer">${ICON.edit}</button>
          <button class="btn sm ghost" onclick="clone(${j.id})" title="Cloner" aria-label="Cloner">${ICON.clone}</button>
          <button class="btn sm ghost dgr" onclick="del(${j.id})" title="Supprimer" aria-label="Supprimer">${ICON.trash}</button>
        </div>
      </div>
      <div class="job-meta">
      ...
      <div class="hist" id="hist-${j.id}"></div>
...
async function scanImport(){
  ...
  if(!items.length && !found.length){
    box.innerHTML=`<div class="empty" style="padding:32px">
      ...
      <p style="font-size:12px">Aucune tâche cron, unité systemd ou watcher inotify trouvé. Vérifie les capacités hôte et, pour les scripts personnalisés, SB_IMPORT_PATHS.</p></div>`;
    return;
  }
...
// ---- instances distantes ----
let REMOTES=[], RSTAT={}; // RSTAT[id] = "on" | "off" | undefined (inconnu)
...
function renderInstances(){
  ...
  sel.innerHTML='<option value="">🖥 Cette instance</option>'
    +REMOTES.map(r=>{const s=RSTAT[r.id];const dot=s==="on"?"🟢":s==="off"?"🔴":"⚪";
      return `<option value="/api/remote/${r.id}">${dot} ${esc(r.name)}</option>`;}).join("");
  sel.value=cur;
}
// heartbeat : ping chaque instance distante (via le proxy) pour l'état en ligne.
async function pingRemotes(){
  await Promise.all(REMOTES.map(async r=>{
    try{ const x=await _rawFetch("/api/remote/"+r.id+"/api/engines"); RSTAT[r.id]=x.ok?"on":"off"; }
    catch(e){ RSTAT[r.id]="off"; }
  }));
  renderInstances();
}
...
// ---- réglages (notifications d'échec) ----
async function openSettings(){
  $("#settings-scrim").classList.add("open");
  try{ const s=await (await _rawFetch("/api/settings")).json();
    $("#s-url").value=s.notifyUrl||"";
    const sw=$("#s-all"); sw.dataset.on=s.notifyAll?"1":""; sw.classList.toggle("on",!!s.notifyAll);
  }catch(e){}
}
function closeSettings(){ $("#settings-scrim").classList.remove("open"); }
function toggleNotifyAll(){ const el=$("#s-all"); const on=el.dataset.on!=="1"; el.dataset.on=on?"1":""; el.classList.toggle("on",on); }
async function saveSettings(test){
  const body={notifyUrl:$("#s-url").value.trim(),notifyAll:$("#s-all").dataset.on==="1",test:!!test};
  const r=await _rawFetch("/api/settings",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});
  if(!r.ok)return toast(await r.text(),"err");
  const d=await r.json();
  if(test) toast(d.testError?("Test échoué : "+d.testError):"Notification de test envoyée ✅", d.testError?"err":"ok");
  else toast("Réglages enregistrés","ok");
}

// ---- historique des runs (par job) ----
async function toggleHist(id){
  const box=$("#hist-"+id); if(!box)return;
  if(box.innerHTML){ box.innerHTML=""; return; } // 2e clic = referme
  let h=[]; try{ h=await (await fetch("/api/jobs/"+id+"/history")).json(); }catch(e){}
  if(!h.length){ box.innerHTML='<div class="hint" style="padding:2px 0">Aucun run enregistré pour l\'instant.</div>'; return; }
  box.innerHTML=h.map(e=>`<div class="hist-row"><span class="hd ${e.status==="ok"?"ok":e.status==="error"?"error":""}">${esc(e.status)}</span><span>${esc(e.ts)}</span><span class="hdur">${e.dur}s</span>${e.note?`<span class="hnote" title="${esc(e.note)}">${esc(e.note)}</span>`:""}</div>`).join("");
}

// ---- UX : clavier (cards/switches), Échap + clic fond pour fermer les modales ----
function initUX(){
  document.querySelectorAll('.card,.sw').forEach(el=>{ el.setAttribute('role','button'); if(!el.hasAttribute('tabindex')) el.setAttribute('tabindex','0'); });
  document.addEventListener('keydown',e=>{
    if(e.key==='Escape'){ document.querySelectorAll('.scrim.open').forEach(s=>s.classList.remove('open')); return; }
    if(e.key==='Enter'||e.key===' '){
      const a=document.activeElement;
      if(a&&(a.classList.contains('card')||a.classList.contains('sw'))){ e.preventDefault(); a.click(); }
    }
  });
...
function openEditor(){
  ...
  ["f-id","f-name","f-command","f-source","f-dest","f-cron","f-watchglob","f-watchsrc","f-debounce","f-pollsec","f-timeout","f-bwlimit","f-maxdel","f-exclude","f-backupkeep","f-user","f-uid","f-group","f-gid"].forEach(i=>$("#"+i).value="");
...
function edit(id){
  ...
  $("#f-user").value=(j.identity&&j.identity.user)||""; $("#f-uid").value=(j.identity&&Number.isInteger(j.identity.uid))?j.identity.uid:"";
  $("#f-group").value=(j.identity&&j.identity.group)||""; $("#f-gid").value=(j.identity&&Number.isInteger(j.identity.gid))?j.identity.gid:"";
...
async function saveJob(){
  ...
  const p={
    ...
    identity:{mode:"fixed",user:$("#f-user").value.trim(),uid:Number($("#f-uid").value),group:$("#f-group").value.trim(),gid:Number($("#f-gid").value)},
  ...
  if(!p.identity.user||!p.identity.group||$("#f-uid").value===""||$("#f-gid").value===""||p.identity.uid<0||p.identity.gid<0) return toast("Renseigne l’utilisateur, l’UID, le groupe et le GID de l’hôte","err");
  const id=$("#f-id").value;
...
const I18N_EN = {
  ...
  "Lance une commande ou un script (ex. docker system prune -f, /srv/scripts/maintenance.sh). Déclenchable en manuel, cron ou surveillance.":"Runs a command or script (e.g. <b>docker system prune -f</b>, <b>/srv/scripts/maintenance.sh</b>). Trigger it manually, on cron, or on a folder change.",
  "Commande à exécuter":"Command to run",
  "Exécutée via sh -c sur l’hôte sous l’identité sélectionnée. stdout/stderr sont capturés et streamés dans les logs (bouton « Lancer »). Les commandes Docker utilisent le CLI/socket de l’hôte via le namespace hôte ; aucun socket Docker n’est monté dans SyncBridge.":"Runs via <b>sh -c</b> on the host under the selected identity. <b>stdout/stderr</b> are captured and streamed to the logs (Run button). Docker commands use the host CLI/socket through the host namespace; no Docker socket is mounted into SyncBridge.",
  ...
  "Écrit un vrai cron/systemd sur l'hôte via le host executor : le job survit à une panne Docker. Demande un déclencheur cron ou surveillance (pas manuel).":"Writes a real host cron/systemd entry through the host executor: the job <b>survives a Docker outage</b>. Requires a cron or watch trigger (not manual).",
  ...
  "Copie système fidèle : préserve hardlinks, ACL, xattrs et identités numériques avec -aHAX --numeric-ids --fake-super. Les métadonnées privilégiées sont stockées dans les xattrs de destination afin de permettre une restauration fidèle, même lorsque le job s’exécute sous une identité hôte non-root.":"Faithful system copy: preserves hardlinks, ACLs, xattrs, and numeric identities with <b>-aHAX --numeric-ids --fake-super</b>. Privileged metadata is stored in destination xattrs so the backup can be restored faithfully even when the job runs under a non-root host identity.",
  ...
  "SyncBridge scanne les crontabs de l’hôte et les chemins configurés par SB_IMPORT_PATHS pour trouver les commandes rsync et rclone. Chaque copie trouvée peut être importée comme job (créé désactivé — tu vérifies puis actives). L'original n'est pas touché.":"SyncBridge scans host crontabs and paths configured with <b>SB_IMPORT_PATHS</b> to find <b>rsync</b> and <b>rclone</b> commands. Each copy found can be imported as a job (created <b>disabled</b> — review, then enable). The original is untouched.",
  ...
  "Réglages":"Settings","Notifications":"Notifications","URL de notification":"Notification URL",
  "Compatible ntfy (topic) et la plupart des webhooks : SyncBridge envoie un POST texte avec un header Title. Vide = désactivé.":"Works with ntfy (topic) and most webhooks: SyncBridge sends a text POST with a Title header. Empty = disabled.",
  "Notifier aussi les succès":"Also notify on success",
  "Par défaut, seuls les échecs déclenchent une notification.":"By default only failures trigger a notification.",
  "Tester":"Test","Enregistrer":"Save","Historique":"History",
  "Aucun run enregistré pour l'instant.":"No runs recorded yet."
};
